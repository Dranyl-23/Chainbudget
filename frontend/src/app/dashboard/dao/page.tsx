    ...
    <div className="p-4 md:p-8 pb-20 animate-fade-in space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {proposals.length === 0 ? (
          ...
          proposals.map(p => (
            <div key={p._id} className="glass p-6 rounded-2xl flex flex-col hover:-translate-y-1 transition-transform duration-300">
              <div className="flex justify-between items-start mb-4">
                ...
                {p.blockchainProposalId && (
                  <span className="text-xs text-primary flex items-center gap-1 font-medium bg-primary/10 px-2 py-1 rounded-full">
                ...
                <p className="text-sm text-gray-500 mb-4 line-clamp-3 flex-1">{p.description}</p>
                ...
                <div className="bg-gray-50/50 p-4 rounded-xl mb-6">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-500">Proposed By</span>
                    <span className="text-sm font-medium text-gray-700">{getCreatorLabel(p.creator)}</span>
                  </div>
                </div>
                ...
                {p.status === "active" && !p.hasVoted && canInteract && (
                  <div className="grid grid-cols-2 gap-3 mt-auto">
                    <button 
                      ...
                      className="btn-primary bg-emerald-500 hover:bg-emerald-600 border-none w-full"
                    >
                    ...
                    <button 
                      ...
                      disabled={votingOn === p._id}
                      className="btn-outline border-red-200 text-red-600 hover:bg-red-50 w-full"
                    >
                ...
                {p.hasVoted && (
                  <div className="mt-auto bg-gray-100 text-gray-500 text-center py-2 rounded-xl text-sm font-medium flex items-center justify-center gap-2">
                ...
                <div className="mt-4 pt-4 border-t border-gray-100 flex gap-4">
                  <div className="flex-1 bg-emerald-50/50 p-2 rounded-lg text-center border border-emerald-100/50">
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-emerald-600 block mb-1">Yes Votes</span>
                    <span className="text-xl font-bold text-emerald-700">{p.yesVotes}</span>
                  </div>
                  <div className="flex-1 bg-red-50/50 p-2 rounded-lg text-center border border-red-100/50">
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-red-600 block mb-1">No Votes</span>
                    <span className="text-xl font-bold text-red-700">{p.noVotes}</span>
                  </div>
                </div>
              </div>
            ))
