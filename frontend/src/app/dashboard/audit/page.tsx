        ...
        <Portal>
          <div id="printable-report" className="bg-white text-black font-sans hidden print:block p-8">
            <div className="flex justify-between items-start border-b-2 border-gray-800 pb-4 mb-6">
              <div>
                <h1 className="text-3xl font-black text-gray-900 mb-1 tracking-tight">CHAIN<span className="text-primary">BUDGET</span></h1>
              ...
              <tbody className="text-gray-700 divide-y divide-gray-200">
                {logs.map(log => (
                  <tr key={log._id}>
                    ...
                    <td className="py-3 pr-2 align-top capitalize">{log.action.replace("transaction.", "").replace("user.", "").replace("_", " ")}</td>
                    <td className="py-3 pr-2 align-top text-[9px]">{formatDetails(log.details)}</td>
                    <td className="py-3 align-top font-mono text-[9px] break-all text-primary">{log.blockchainTxHash || "Off-chain event"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
